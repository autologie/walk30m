export default {
  ...
  'descriptionTpl': `Walk30m（${process.env.APP_URL}）から<%= executionDateExpr %>にエクスポート.  開始地点: <%= originAddress %>（緯度: <%= origin.lat %>, 経度: <%= origin.lng %>）, 所要時間: <%= timeExpr %>分, 交通手段: <%= travelModeExpr %>, 計算時の優先項目: <%= preferenceExpr %>, 有料道路の使用: <%= avoidTollsExpr %>, 自動車専用道路の使用: <%= avoidHighwaysExpr %>, フェリーの使用: <%= avoidFerriesExpr %>`,
