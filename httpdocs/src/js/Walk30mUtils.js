...
class Walk30mUtils {
  ...
  static encodeResult(coords) {
    const diffSequence = coords.reduce(
      (passed, elem) => [
        elem,
        passed[1].concat([
          [
            Math.round(
              1000000 * (elem.lng - (passed[0] === null ? 0 : passed[0].lng))
            ),
            Math.round(
              1000000 * (elem.lat - (passed[0] === null ? 0 : passed[0].lat))
            )
          ]
        ])
      ],
    ...
    return diffSequence
      .map(s => [s[0].toString(36), s[1].toString(36)].join(" "))
      .join(",")
      .replace(/,/g, " ")
      .replace(/\s/g, "+")
      .replace(/\+-/g, "-");
  ...
  static createSharedURI(feature) {
    try {
      ...
      const encoded = JSON.stringify(
        feature.getProperty("task").serialize().config
      );
      const reqExpr = window.encodeURIComponent(encoded);
      const pathExpr = Walk30mUtils.encodeResult(
        path.map(latLng => latLng.toJSON())
      );
