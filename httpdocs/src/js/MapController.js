...
class MapController {
    ...
    this.showMessage(
      calcMsgTpl(
        _.defaults(
          {
            min: request.time / 60,
            travelModeExpr: this.application.getMessage("travelModes")[
              request.mode
            ]
          },
          request
  ...
  specifyLocation(callback) {
    ...
    this.$cancelBtn
      .off()
      .one("click", () => this.finalize(dragStartListener, callback));
    this.$determineBtn
      .off()
      .one("click", () =>
        this.finalize(dragStartListener, () => callback(this.map.getCenter()))
      );
  }
}
